	...
	<div class="card-body">
		<div class="row">
			<div class="col-lg-12" style="margin:1%">
				<textarea v-on:keyup="scan()" class="form-control" rows="20" v-model="answerText" id="target" v-html="answerText"></textarea>
			</div>
		</div>
	</div>
	<div class="card-footer" align="center"><button type="button" v-on:click.native="editAnswer" class="btn btn-sm bg-dark text-white">திருத்துக</button></div>
</div>
<script type="text/javascript">
	const componentVar = {
		...
		methods:{
			...
			editAnswer:function(){
				if (this.answerText=="") {
					...
					answerData.append("answerID","<?php echo $_GET['answerID']; ?>");
					answerData.append("editedAnswer",this.answerText);
					fetch("<?php echo base_url('index.php/answer/updateAnswer'); ?>",{
						method:'POST',
						body: answerData,
						mode: 'no-cors',
						cache: 'no-cache'})
					...
					.then(data => {
						alert(data.message);
						window.location.href="<?php echo base_url('index.php/question/viewSingleQuestionWithAnswers?questionid='); ?>"+data.questionid;
					})
					.catch(() => {
						(console.log("Network connection error"));
		...
		mounted(){
			...
			xhttp.open("GET", "<?php echo base_url('index.php/answer/deriveAnswerForEditing?answerID='.$_GET['answerID']); ?>", true);
			xhttp.send();
		},
		updated(){
			var innerThis=this;
